class rv_sub extends uvm_subscriber#(rv_txn_sub);
  ...
  bit stall, bubble, burst, backpresure_on_last;
  int unsigned stall_len = 0, burst_len = 0, pkt_len = 0;
  ...
  function void build_phase(uvm_phase phase);
    ...
    cg = new(.MAX_BURST_LEN(MAX_BURST_LEN));
  endfunction
  ...
  function void write(rv_txn_sub t);
    ...
    //stall_len, backpresure_on_last
    stall_len = stall? stall_len+1: 0;
    backpresure_on_last = (stall && t.last)? 1: 0;
    //burst_len
    burst_len = burst?  burst_len+1: 0;
    
    //pkt_len
    pkt_len = (burst && t.last)? pkt_len+1: pkt_len;
